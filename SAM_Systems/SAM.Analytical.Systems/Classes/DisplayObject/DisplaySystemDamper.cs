...
{
    public class DisplaySystemDamper : SystemDamper, IDisplaySystemObject<SystemGeometryInstance>
    {
        ...
        public DisplaySystemDamper(SystemDamper systemDamper, SystemGeometrySymbol systemGeometrySymbol, Point2D location)
            :base(systemDamper)
        {
            systemGeometryInstance = new SystemGeometryInstance(systemGeometrySymbol, location);
        }

        public DisplaySystemDamper(DisplaySystemDamper displaySystemDamper)
            : base(displaySystemDamper)
        {
            systemGeometryInstance = displaySystemDamper?.systemGeometryInstance == null ? null : new SystemGeometryInstance(displaySystemDamper?.systemGeometryInstance);
        }

        public DisplaySystemDamper(System.Guid guid, DisplaySystemDamper displaySystemDamper)
            : base(guid, displaySystemDamper)
        {
            systemGeometryInstance = displaySystemDamper?.systemGeometryInstance == null ? null : new SystemGeometryInstance(displaySystemDamper?.systemGeometryInstance);
        }

        public DisplaySystemDamper(JsonObject jObject)
            : base(jObject)
        {
        ...
        {
            return new DisplaySystemDamper(guid == null ? Guid.NewGuid() : guid.Value, this);
        }
    }
}
